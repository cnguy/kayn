...
request(SWAGGER_URL, async (err, res) => {
    if (res) {
        ...
        const resultsPromises = Promise.all(pathsKeys.map(async pathKey => {
            const methodTypes = Object.keys(paths[pathKey])
            const tmp = await Promise.all(methodTypes.map(async k => {
                const targetEndpoint = `${k.toUpperCase()} \`${pathKey}\``
                const f = await findInFiles.find(targetEndpoint, ENDPOINTS_DIR_PATH, '.js')
