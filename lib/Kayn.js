...
const loggerFactory = namespace => {
    console.log(`${loggerPrefix}:${namespace}`);
    return require('debug')(`${loggerPrefix}:${namespace}`);
};
...
class Kayn {
    constructor(
        ...
        if (!ParameterHelper.isKeyValid(key)) {
            throw new Error(
                'Failed to initialize Kayn! API key is not a string.',
            );
        }
        ...
        if (this.config.debugOptions.isEnabled) {
            this.config.debugOptions.loggers.initLogger = initLogger;
            this.config.debugOptions.loggers.request = {
                incoming: {
            ...
            this.config.debugOptions.loggers.cache = {
                set: cacheSetLogger,
                get: cacheGetLogger,
            };
        ...
        if (this.config.debugOptions.isEnabled) {
            this.config.debugOptions.loggers.initLogger('Ready!');
        }
    }
