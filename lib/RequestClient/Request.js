...
Request.prototype.execute = async function(url, body, token, retriesLeft, cb) {
    ...
    const fn = async (err, data) => {
        ...
        try {
            ...
            if (statusCode === 403) {
                console.log('[kayn] Your key is either invalid or you are magically blacklisted. You should not be getting blacklisted with this rate limiter, so something is wrong with this library, the rate limiter, your network, and/or the LoL API! Exiting program...')
                process.exit()
            }
            // rest implies RequestError (generally socket hang ups)
            if (rest || requestOptions.shouldRetry && shouldRetry(statusCode)) {
                if (retriesLeft > 0) {
                    return setTimeout(
                        () =>
